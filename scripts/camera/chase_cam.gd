...
func _process_old(delta):
	...
	ground_velocity *= 0.0
	look_at_dir = look_at_dir.lerp(car.global_position + ground_velocity.normalized() + Vector3.UP*1, delta * 100.0)# + ground_velocity.normalized()
	...
	var target_fov = 70 + abs(speed)*0.4
